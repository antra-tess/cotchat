...
class ChatInterface:
    def __init__(self, context_file: str = None):
        ...
        self.model = os.getenv("DEFAULT_MODEL", "deepseek/deepseek-chat-r1")
    ...
    def cleanup_last_exchange(self):
        """Remove the last message pair (user + assistant) and clear from screen"""
        if not self.messages:
            return
        ...
        # If last message is assistant, remove it first
        if last_role == "assistant":
            self.clear_last_message()
            self.messages.pop()
            
        # Now check again - should be user message
        last_role = self.messages[-1].role if self.messages else None
        if last_role == "user":
            self.clear_last_message()
            self.messages.pop()
        ...
        if self.messages:
            ...
            if last_role == "user":
                console.print("[red]Debug: Cleanup left a user message![/]")
            elif last_role == "assistant":
                # This is okay - it's the previous complete exchange
                pass
    ...
    def get_completion(self):
        try:
            ...
            # Clean up if generation was interrupted or failed
            if not success:
                self.cleanup_last_exchange()
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                message = "Edit message: " if self.edit_mode else "You: "  # Added clear prompt
